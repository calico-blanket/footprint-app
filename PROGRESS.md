...
### 新たな運用方針と機能追加（2026-04-03）
- **方針転換**: Web Share Targetまたは位置情報取得による抜本的解決はコスト面から断念。「アップロードした時点の現在地で登録されることを逆手に取り、写真を撮ったその場でFootprint Appから直接写真を投稿する運用」に変更。時間がない場合は写真のみ登録し、コメント等は後から更新する。
- **機能追加**: `feature/in-app-camera` ブランチを作成し、アプリ内から直接カメラを起動して撮影・アップロードできるボタンを追加機能として実装しました。
...
- **2026-04-05**:
    - **Android PWAでの直接カメラ起動の不具合検証と修正**:
        - PWA環境から直接カメラを起動し、写真を保存する運用（`in-app-camera` ブランチの main への統合）を試行。
        - `<input type="file" capture="environment">` を利用した実装において、Android環境で撮影終了後に `onChange` イベントが発火せず（画像ファイルが返却されず）、プレビューが表示されない事象が発生。
        - プログラマティックな `click()` から `<label>` 要素によるネイティブクリックへの変更、および無効な `accept` 属性や競合する `onClick` イベントの削除など数々のデバッグと修正を実施。
        - 最終的に、Androidブラウザの `capture="environment"` ＋ PWA 環境におけるOSレベルの不具合（ファイルがWebプロセスに返却されないバグ）である可能性が極めて高く、現時点では完全解決に至らず。
        - 引き続き、「標準ファイルピッカーからカメラを選択させる方式」か「WebRTCによる完全自作カメラ」等、根本的な運用および実装方針の見直しを検討中。
