...
- **2026-04-17**:
    - **新規登録画面に「過去の記録からコピー」機能を追加**:

        ### 背景・経緯
        - タグ機能は以前から実装されていたが、タグを「付ける」だけで「活用する」用途がなかった。
        - タイムラインのタグフィルターは既に動作していたが、タグ数が多く目的のタグにたどり着きにくいという課題があった。
        - 新規登録の際に、同じ場所・カテゴリ・タグの定番コンパクションを毎回入力し直す手間を省くため、「過去の記録をテンプレートとして流用する」機能の実装に至った。

        ### 実装内容
        #### 新規コンポーネント: `components/CopyFromRecord.tsx`
        - 新規登録フォーム（`RecordForm.tsx`）の先頭に「過去の記録からコピー」ボタンを追加（編集時は非表示）。
        - ボタンを押すと展開し、以下のUIを提供：
            1. **タグ一覧（チップ形式）**: 全タグをボタン形式で表示。クリックで絞り込みタグを選択。
            2. **タグ全表示トグル**: 初期表示20件を超えるタグは「他XX件を表示」ボタンで全件展開・折りたたみ可能。
            3. **テキスト検索**: 入力文字でタグ候補をリアルタイム絞り込み（サジェスト表示）。
            4. **複数タグ AND 絞り込み**: 選択したタグをすべて含む記録のみを表示。
            5. **過去記録カード**: タグ一致した記録を最新順でカード表示。各カードには写真サムネイル・日付・カテゴリ・メモ・タグを表示。写真がある場合は最大5枚のサムネイルをカード上部に表示。
            6. **「使う」ボタン**: 記録を選択すると確認ダイアログを表示。
            7. **確認ダイアログ**: コピーされる項目（カテゴリ・タグ・位置情報）を提示して確認。

        #### `components/RecordForm.tsx` の変更
        - `loadAllTags` を `loadAllTagsAndRecords` に拡張し、全記録データを `allRecords` ステートとして保持するように変更（日付の新しい順にソート）。
        - `CopyFromRecord` コンポーネントをインポートし、フォーム先頭に配置。
        - コピー時のハンドラ（`onCopy`）でカテゴリ・タグ・位置情報をフォームに反映し、トースト通知を表示。
        ...
        ### Gitブランチと作業フロー
        - ブランチ `feature/tag-copy-from-history` を作成して実装・テスト後、`main` へ Fast-forward マージ。
        - 変更ファイル: `CopyFromRecord.tsx`（新規308行）、`RecordForm.tsx`（32行変更）
